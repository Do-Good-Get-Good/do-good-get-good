...
jest.mock('../context/CreateActivityContext', () => ({
  ...
  it(" Choice from DropDown = Skapa ny aktivitet. Placeholder Aktivity title exist   ", () =>{
   ...
   fireEvent.changeText(getByPlaceholderText("Aktivitet [obligatorisk]"), "Aktivity title");
...
it(" Choice from DropDown = Skapa ny aktivitet. Placeholder Place exist   ", () =>{
 ...
 route.params.creatingNewUser = true
 const { getByPlaceholderText} = render(< CreateActivity route={route} />);
 fireEvent.changeText(getByPlaceholderText("Var [obligatorisk]"), "Aktivity place");
...
it(" Choice from DropDown = Skapa ny aktivitet. Placeholder City exist   ", () =>{
 ...
 route.params.creatingNewUser = true
 const { getByPlaceholderText} = render(< CreateActivity route={route} />);
 fireEvent.changeText(getByPlaceholderText("Aktör [obligatorisk]"), "Aktivity city");
...
it(" Choice from DropDown = Skapa ny aktivitet. Placeholder Description exist   ", () =>{
 ...
 route.params.creatingNewUser = true
 const { getByPlaceholderText} = render(< CreateActivity route={route} />);
 fireEvent.changeText(getByPlaceholderText("Vad [valfritt]"), "Aktivity Description");
...
it( "Photo exists in CreateActivity",() => {
  ...
  const image = getByTestId('photo')
   expect(image.props.source).toEqual({testUri: "../../../img/activities_images/symbol_hands_heart-DEFAULT.png"})
   
    })
    it( "ActivityCard. Navigation to ImagesGallery",() => {
      const { getByTestId }  = render(<CreateActivity route={route } navigation={navigation }/>);
    ...
    it( "ActivityCard. Text 'Infoga' exidst",() => {
      const { getAllByText }  = render(<CreateActivity route={route } navigation={navigation }/>);
    ...
    it( "ActivityCard. Text '[TG-favoriter vissas för alla användare]' exidst",() => {
      const { getAllByText }  = render(<CreateActivity route={route } navigation={navigation }/>);
      expect(getAllByText("[TG-favoriter vissas för alla användare]").length).toBe(1)  
    })
    ...
    it( "ActivityCard. Button 'Spara' send new activity to createActivityContext",() => {
      ...
      const sendNewActivityToCreateActivityContext = getByTestId("sendNewActivityToCreateActivityContext");
